...
# Return the magnitude history of scalar or vector quantities
...
# Compute the discrete integral of acceleration squared
...
# Return the maximum or zero if the history is empty
...
# Return the minimum or zero if the history is empty
...
# Format the time value with a given precision, or return "-" if no interception occurred
...
# Compute interception, acceleration, effort and saturation metrics
def compute_metrics(results):
    time = np.asarray(results["time"], dtype=float)
    R = np.asarray(results["R"], dtype=float)

    # Command generated by the guidance law before implementation limits
    ...
    # Total pursuer acceleration, including guidance and longitudinal acceleration
    aM_total = np.asarray(results.get("aM_total", a_applied), dtype=float)

    # Signed longitudinal acceleration along the pursuer velocity direction
    ...
    # Boolean history indicating whether the lateral command was saturated
    ...
    # Capture and closest approach may occur at different instants
    if len(R) > 0:
    ...
    # Verify capture from range history when capture radius is available
    ...
    # Magnitudes used to compare acceleration demands
    ...
    # Effort metrics approximate the time integral of acceleration squared
    ...
    # Saturation metrics quantify how often the requested command exceeded the limit
...
# Print a compact comparison table for multiple guidance laws
