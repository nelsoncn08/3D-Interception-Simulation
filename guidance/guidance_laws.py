...
def relative_quantities(rM, vM, rT, vT):
    ...
    if R < 1e-9:
        ...
        # Still not sure what to do if more attempts are desired... maybe a different guidance law should be used, but for now I'll comment the if statement below
#        if Vc < 0.0:
#            Vc = 0.0
...
# Remove the acceleration component parallel to the pursuer velocity (just so that the guidance law is not affected by the pursuer speed)
...
def true_proportional_navigation(rM, vM, rT, vT, N):
    _, _, R, lambda_hat, Vc, omega_los = relative_quantities(rM, vM, rT, vT)
...
def pure_proportional_navigation(rM, vM, rT, vT, N):
    _, _, R, _, _, omega_los = relative_quantities(rM, vM, rT, vT)
    ...
    # In PPN, the command is normal to the pursuer velocity
